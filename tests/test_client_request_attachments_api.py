...
def user(db):
    return User.objects.create_user(
        username="client_attach_user", password="pass123", email="u@example.com", is_staff=True
    )
...
def project(db):
    return Project.objects.create(
        name="ClientAttachProj", client="ACME", start_date=date.today(), address="123 Main St"
    )
...
def test_client_request_attachments_upload_and_list(api_client, user, project):
    ...
    up = api_client.post(
        "/api/v1/client-request-attachments/",
        {"request": cr_id, "file": upload, "filename": "specs.txt", "content_type": "text/plain"},
        format="multipart",
    )
