...
from core.models import DailyPlan, Employee, PlannedActivity, Project, Task, TimeEntry
...
class TestVarianceAndPlanRollup:
    ...
    def test_daily_plan_recompute_actual_hours(self, api_client, user, employee, project):
        ...
        act1.save()
        act2.converted_task = t2
        act2.save()
        # Time entries sum to 7.00
        TimeEntry.objects.create(
            employee=employee, project=project, task=t1, date=date.today(), start_time=time(8, 0), end_time=time(11, 0)
        )  # 3h
        TimeEntry.objects.create(
            employee=employee, project=project, task=t2, date=date.today(), start_time=time(12, 0), end_time=time(16, 0)
        )  # 4h
        # Recompute
