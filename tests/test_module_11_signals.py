...
def admin_user(db):
    ...
    return User.objects.create_user(
        username="admin_signals", email="admin_sig@test.com", password="admin123", is_staff=True, is_superuser=True
    )
...
def user(db):
    ...
    return User.objects.create_user(username="user_signals", email="user_sig@test.com", password="user123")
...
def project(db):
    ...
    return Project.objects.create(
        name="Test Project Signals", client="Test Client", start_date=date.today(), budget_total=Decimal("10000.00")
    )
...
class TestTaskStatusNotifications:
    ...
    def test_notify_creator_on_status_change(self, project, user, admin_user):
        ...
        task = Task.objects.create(project=project, title="Test Task", created_by=user)
        ...
        notifications = Notification.objects.filter(
            user=user, notification_type="task_completed", related_object_type="Task", related_object_id=task.pk
        )
...
class TestTaskStatusNotifications:
    ...
    def test_notify_creator_on_status_change(self, project, user, admin_user):
        ...
        from core.models import Employee, Notification, Task
        ...
        # Create task with user as creator
        task = Task.objects.create(project=project, title="Test Task", created_by=user, assigned_to=employee)
