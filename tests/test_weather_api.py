...
class TestWeatherAPI:
    ...
    def test_weather_snapshot_successful_api_call(self, mocker):
        ...
        # Create test project
        project = Project.objects.create(name="Test Weather Project", start_date=timezone.now().date())
    ...
    def test_weather_snapshot_api_error_handling(self, mocker):
        ...
        project = Project.objects.create(name="Test Error Project", start_date=timezone.now().date())
    ...
    def test_weather_snapshot_updates_existing(self, mocker):
        ...
        # Use date.today() to match task behavior (localdate)
        today = date.today()
        project = Project.objects.create(name="Update Test Project", start_date=today)
    ...
    def test_weather_codes_mapping(self, mocker):
        ...
        project = Project.objects.create(name="Weather Code Test", start_date=timezone.now().date())
    ...
    def test_weather_api_only_active_projects(self, mocker):
        ...
        active1 = Project.objects.create(name="Active 1", start_date=today)

        # Active project (end date in future)
        active2 = Project.objects.create(name="Active 2", start_date=today, end_date=today + timedelta(days=30))
    ...
    def test_weather_api_parameters(self, mocker):
        ...
        project = Project.objects.create(name="Param Test", start_date=timezone.now().date())
    ...
    def test_humidity_estimation_logic(self, mocker):
        ...
        project = Project.objects.create(name="Humidity Test", start_date=timezone.now().date())
...
class TestWeatherSnapshotModel:
    ...
    def test_weather_snapshot_creation(self):
        ...
        project = Project.objects.create(name="Snapshot Test", start_date=timezone.now().date())
    ...
    def test_weather_snapshot_raw_json_storage(self):
        ...
        project = Project.objects.create(name="JSON Test", start_date=timezone.now().date())

        raw_data = {"latitude": 37.7749, "longitude": -122.4194, "weather_code": 61, "temperature_max": 22.5}

        snapshot = WeatherSnapshot.objects.create(
            project=project, date=timezone.now().date(), source="open-meteo", temperature_max=22.5, raw_json=raw_data
        )
