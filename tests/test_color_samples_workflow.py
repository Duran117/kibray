...
from core.models import ClientProjectAccess, ColorSample, Project
...
def admin_user(db):
    return User.objects.create_user(
        username="admin_colors", password="pass123", email="admin@example.com", is_staff=True
    )
...
def regular_user(db):
    return User.objects.create_user(
        username="client_colors", password="pass123", email="client@example.com", is_staff=False
    )
...
def project(db):
    return Project.objects.create(
        name="Color Approval Project", client="KPIS Construction", start_date=date.today(), address="456 Design Ave"
    )
...
def test_sample_number_sequential(api_client, admin_user, project):
    ...
    for i in range(3):
        ...
        res = api_client.post(
            "/api/v1/color-samples/",
            {"project": project.id, "name": f"Sample {i}", "code": f"CODE{i}", "sample_image": upload},
            format="multipart",
        )
...
def test_sample_number_unique_per_project(api_client, admin_user):
    ...
    res1 = api_client.post(
        "/api/v1/color-samples/",
        {"project": proj1.id, "name": "P1 Sample", "sample_image": upload1},
        format="multipart",
    )
    ...
    res2 = api_client.post(
        "/api/v1/color-samples/",
        {"project": proj2.id, "name": "P2 Sample", "sample_image": upload2},
        format="multipart",
    )
...
def test_approve_color_sample(api_client, admin_user, project):
    ...
    sample = ColorSample.objects.create(
        project=project, name="Approved Color", code="SW1234", status="proposed", created_by=admin_user
    )

    # Approve
    res = api_client.post(f"/api/v1/color-samples/{sample.id}/approve/", {"signature_ip": "192.168.1.100"})
...
def test_reject_color_sample_with_reason(api_client, admin_user, project):
    ...
    # Create sample
    sample = ColorSample.objects.create(
        project=project, name="Rejected Color", code="SW5678", status="review", created_by=admin_user
    )

    # Reject with reason
    res = api_client.post(
        f"/api/v1/color-samples/{sample.id}/reject/", {"reason": "Color does not match design requirements"}
    )
...
def test_reject_without_reason_fails(api_client, admin_user, project):
    ...
    sample = ColorSample.objects.create(project=project, name="Test Sample", status="proposed", created_by=admin_user)
...
def test_request_changes(api_client, admin_user, project):
    ...
    sample = ColorSample.objects.create(project=project, name="Under Review", status="proposed", created_by=admin_user)

    res = api_client.post(
        f"/api/v1/color-samples/{sample.id}/request-changes/", {"notes": "Please make it slightly darker"}
    )
...
def test_cannot_approve_already_approved(api_client, admin_user, project):
    ...
    sample = ColorSample.objects.create(
        project=project, name="Already Approved", status="approved", approved_by=admin_user, created_by=admin_user
    )
...
def test_cannot_reject_already_rejected(api_client, admin_user, project):
    ...
    res = api_client.post(f"/api/v1/color-samples/{sample.id}/reject/", {"reason": "New reason"})
...
def test_update_sample_metadata(api_client, admin_user, project):
    ...
    sample = ColorSample.objects.create(project=project, name="Original Name", created_by=admin_user)

    res = api_client.patch(
        f"/api/v1/color-samples/{sample.id}/", {"name": "Updated Name", "notes": "Added notes"}, format="json"
    )
...
def test_delete_sample(api_client, admin_user, project):
    ...
    sample = ColorSample.objects.create(project=project, name="To Delete", created_by=admin_user)
...
def test_create_without_project_fails(api_client, admin_user):
    ...
    res = api_client.post("/api/v1/color-samples/", {"name": "No Project Sample"}, format="json")
...
def test_pagination_enabled(api_client, admin_user, project):
    ...
    # Create 30 samples
    for i in range(30):
        ColorSample.objects.create(project=project, name=f"Sample {i}", created_by=admin_user)
