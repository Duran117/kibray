...
from core.models import AuditLog, LoginAttempt, PermissionMatrix, Project
...
class TestPermissionMatrix:
    ...
    def test_create_permission_matrix(self):
        ...
        project = Project.objects.create(name="Test Project", start_date=timezone.now().date(), budget_total=10000)
    ...
    def test_permission_api_check_permission(self):
        ...
        PermissionMatrix.objects.create(
            user=user, role="contractor", entity_type="expense", can_view=True, can_create=True, can_edit=False
        )
        ...
        response = client.get("/api/v1/permissions/check_permission/", {"entity_type": "expense", "action": "create"})
        assert response.status_code == 200
        assert response.json()["has_permission"] is True

        # Check denied permission
        response = client.get("/api/v1/permissions/check_permission/", {"entity_type": "expense", "action": "delete"})
        assert response.status_code == 200
...
class TestAuditLog:
    ...
    def test_audit_log_change_tracking(self):
        ...
        new_values = {"status": "completed", "budget": "12500"}
    ...
    def test_audit_log_api_entity_history(self):
        ...
        response = client.get("/api/v1/audit-logs/entity_history/", {"entity_type": "project", "entity_id": 123})
        assert response.status_code == 200
...
class TestLoginAttempt:
    ...
    def test_log_successful_login(self):
        ...
        attempt = LoginAttempt.log_attempt(
            username="testuser", ip_address="10.0.0.1", success=True, user_agent="Mozilla/5.0"
        )
    ...
    def test_rate_limit_not_exceeded(self):
        ...
        for _ in range(3):
            LoginAttempt.log_attempt(username=username, ip_address=ip, success=False, failure_reason="invalid_password")
    ...
    def test_rate_limit_window_expires(self):
        ...
        for _ in range(5):
            attempt = LoginAttempt.objects.create(
                username=username, ip_address=ip, success=False, failure_reason="invalid_password"
            )
    ...
    def test_login_attempt_api_recent_failures(self):
        ...
        for i in range(3):
            LoginAttempt.log_attempt(
                username=user.username, ip_address=f"10.0.0.{i}", success=False, failure_reason="invalid_password"
            )
...
class TestPermissionIntegration:
    ...
    def test_permission_scoped_to_project(self):
        ...
        project1 = Project.objects.create(name="Project 1", start_date=timezone.now().date(), budget_total=10000)
        project2 = Project.objects.create(name="Project 2", start_date=timezone.now().date(), budget_total=20000)

        # Grant permission only for project1
        PermissionMatrix.objects.create(
            user=user, role="contractor", entity_type="task", can_view=True, can_edit=True, scope_project=project1
        )
        ...
        response = client.get(
            "/api/v1/permissions/check_permission/",
            {"entity_type": "task", "action": "edit", "project_id": project1.id},
        )
        assert response.status_code == 200
        assert response.json()["has_permission"] is True
        ...
        response = client.get(
            "/api/v1/permissions/check_permission/",
            {"entity_type": "task", "action": "edit", "project_id": project2.id},
        )
        assert response.status_code == 200
