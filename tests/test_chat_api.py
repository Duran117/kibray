...
@pytest.fixture
def project(db):
    return Project.objects.create(name="ChatTestProject", client="ACME", start_date=date.today(), address="123 Test St")


@pytest.fixture
def project_access(db, regular_user, project):
    """Grant regular_user access to project"""
    return ClientProjectAccess.objects.create(user=regular_user, project=project, role="client")


@pytest.fixture
def channel(db, project, admin_user):
    ch = ChatChannel.objects.create(name="General", project=project, created_by=admin_user)
...
def test_chat_channel_crud(api_client, admin_user, project):
    ...
    # CREATE
    res = api_client.post(
        "/api/v1/chat/channels/", {"project": project.id, "name": "Dev Team", "channel_type": "group"}, format="json"
    )
    ...
    # UPDATE
    res = api_client.patch(f"/api/v1/chat/channels/{channel_id}/", {"name": "Development Team"}, format="json")
    assert res.status_code == 200
...
def test_channel_add_remove_participants(api_client, admin_user, regular_user, channel):
    ...
    # Add participant
    res = api_client.post(
        f"/api/v1/chat/channels/{channel.id}/add_participant/", {"user_id": regular_user.id}, format="json"
    )
    assert res.status_code == 200
    ...
    # Remove participant
    res = api_client.post(
        f"/api/v1/chat/channels/{channel.id}/remove_participant/", {"user_id": regular_user.id}, format="json"
    )
    assert res.status_code == 200
...
def test_chat_message_crud(api_client, admin_user, channel):
    ...
    # UPDATE
    res = api_client.patch(f"/api/v1/chat/messages/{msg_id}/", {"message": "Hello team! Updated."}, format="json")
    assert res.status_code == 200
...
def test_mention_parsing_user(api_client, admin_user, mentioned_user, channel):
    ...
    notif = Notification.objects.filter(user=mentioned_user, notification_type="chat_message")
    assert notif.exists()
...
def test_mention_parsing_entity_linking(api_client, admin_user, channel, project):
    ...
    task = Task.objects.create(project=project, title="Paint walls")
...
def test_chat_message_with_image_attachment(api_client, admin_user, channel):
    ...
    res = api_client.post(
        "/api/v1/chat/messages/",
        {"channel": str(channel.id), "message": "Check out this photo", "image": upload},
        format="multipart",
    )
...
def test_chat_message_with_file_attachment(api_client, admin_user, channel):
    ...
    res = api_client.post(
        "/api/v1/chat/messages/",
        {"channel": channel.id, "message": "Report attached", "attachment": upload},
        format="multipart",
    )
    assert res.status_code in (200, 201)
