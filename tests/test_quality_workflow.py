...
def test_task_auto_in_review_on_100_progress():
    ...
    task = Task.objects.create(project=project, title="QC Task", assigned_to=emp, status="En Progreso", progress_percent=0)
    task.progress_percent = 100
    task.save()
    assert task.status == "En Revisión"
...
def test_task_approve_sets_completed_and_visible():
    ...
    task = Task.objects.create(project=project, title="QC Approve", status="En Revisión", progress_percent=100)
...
def test_task_reject_sets_in_progress_and_increments_profile_rejections():
    ...
    task = Task.objects.create(project=project, title="QC Reject", assigned_to=emp, status="En Revisión", progress_percent=100)
...
def test_task_dependency_warning_flag():
    project = Project.objects.create(name="QC Project", start_date=timezone.now().date())
    t1 = Task.objects.create(project=project, title="Predecessor", status="Pendiente")
    t2 = Task.objects.create(project=project, title="Successor", status="Pendiente")
