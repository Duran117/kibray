...
def test_color_sample_submit_and_approve(api_client, user, project):
    ...
    # Approve
    res = api_client.post(
        f"/api/v1/color-samples/{sample.id}/approve/", {"signature_ip": "203.0.113.10"}, format="json"
    )
...
def test_color_sample_reject_requires_reason(api_client, user, project):
    ...
    res2 = api_client.post(
        f"/api/v1/color-samples/{sample.id}/reject/", {"reason": "Does not match spec"}, format="json"
    )
