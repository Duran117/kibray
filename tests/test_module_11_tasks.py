...
from core.models import Employee, Project, Task, TaskImage, TaskStatusChange
...
def pm_user(db):
    ...
    user = User.objects.create_user(username="pm", email="pm@test.com", password="pm123")
...
def client_user(db):
    """Client user"""
    user = User.objects.create_user(username="client", email="client@test.com", password="client123")
    if hasattr(user, "profile"):
...
def project(db):
    ...
    return Project.objects.create(
        name="Test Project", client="Test Client", start_date=date.today(), budget_total=Decimal("10000.00")
    )
...
class TestTaskCRUD:
    ...
    def test_create_task_minimal(self, project, admin_user):
        ...
        task = Task.objects.create(project=project, title="Minimal Task", created_by=admin_user)
...
class TestTaskPriorities:
    ...
    def test_default_priority(self, project, admin_user):
        ...
        task = Task.objects.create(project=project, title="No Priority Task", created_by=admin_user)
        assert task.priority == "medium"

    def test_filter_by_priority(self, project, admin_user):
        """Test filtering tasks by priority"""
        Task.objects.create(project=project, title="High Priority", priority="high", created_by=admin_user)
        Task.objects.create(project=project, title="Low Priority", priority="low", created_by=admin_user)
...
class TestTaskDependencies:
    ...
    def test_add_dependency(self, project, admin_user):
        ...
        task1 = Task.objects.create(project=project, title="Task 1", created_by=admin_user)
        task2 = Task.objects.create(project=project, title="Task 2 (depends on Task 1)", created_by=admin_user)
...
class TestTaskDueDate:
    ...
    def test_task_without_due_date(self, project, admin_user):
        ...
        task = Task.objects.create(project=project, title="No Due Date Task", created_by=admin_user)
        assert task.due_date is None

    def test_task_with_due_date(self, project, admin_user):
        """Test task with due date"""
        due = date.today() + timedelta(days=7)
        task = Task.objects.create(project=project, title="Task with Due Date", due_date=due, created_by=admin_user)
...
class TestTaskTimeTracking:
    ...
    def test_touchup_no_tracking(self, project, admin_user):
        ...
        touchup = Task.objects.create(project=project, title="Touch-up Task", is_touchup=True, created_by=admin_user)
...
class TestTouchUpSeparation:
    ...
    def test_create_regular_task(self, project, admin_user):
        ...
        task = Task.objects.create(project=project, title="Regular Task", created_by=admin_user)
    ...
    def test_filter_touchups(self, project, admin_user):
        ...
        Task.objects.create(project=project, title="Regular", is_touchup=False, created_by=admin_user)
        Task.objects.create(project=project, title="Touch-up", is_touchup=True, created_by=admin_user)
...
class TestTaskImageVersioning:
    ...
    def test_upload_first_image(self, task, admin_user):
        ...
        img = TaskImage.objects.create(
            task=task, image="tasks/test_image.jpg", uploaded_by=admin_user, version=1, is_current=True
        )
    ...
    def test_upload_new_version(self, task, admin_user):
        ...
        img1 = TaskImage.objects.create(
            task=task, image="tasks/test_v1.jpg", uploaded_by=admin_user, version=1, is_current=True
        )

        # New version
        img2 = TaskImage.objects.create(
            task=task, image="tasks/test_v2.jpg", uploaded_by=admin_user, version=2, is_current=True
        )
...
class TestTaskPermissions:
    ...
    def test_pm_can_delete(self, project, pm_user):
        ...
        task = Task.objects.create(project=project, title="PM Task", created_by=pm_user)
    ...
    def test_client_cannot_assign(self, project, client_user, employee):
        ...
        task = Task.objects.create(project=project, title="Client Task", created_by=client_user, is_client_request=True)
...
class TestClientRequests:
    ...
    def test_client_cancel_request(self, project, client_user):
        ...
        task = Task.objects.create(
            project=project, title="Client Request", created_by=client_user, is_client_request=True
        )
...
class TestTaskIntegration:
    ...
    def test_task_with_schedule_item(self, project, admin_user):
        ...
        task = Task.objects.create(
            project=project, title="Paint Living Room", schedule_item=item, created_by=admin_user
        )
    ...
    def test_task_without_schedule(self, project, admin_user):
        ...
        task = Task.objects.create(project=project, title="Standalone Task", created_by=admin_user)
...
class TestTaskEdgeCases:
    ...
    def test_empty_title_validation(self, project, admin_user):
        ...
        task = Task(project=project, title="", created_by=admin_user)  # Empty title
