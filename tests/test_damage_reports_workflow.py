...
from core.models import (
    Project, DamageReport, DamagePhoto, FloorPlan, PlanPin,
    ChangeOrder, Task, Notification, ClientProjectAccess
)
...
def test_create_damage_report(api_client, admin_user, project):
    ...
    res = api_client.post('/api/v1/damage-reports/', {
        'project': project.id,
        'title': 'Cracked Tile',
...
def test_update_damage_report(api_client, admin_user, damage_report):
    ...
    res = api_client.patch(f'/api/v1/damage-reports/{damage_report.id}/', {
        'description': 'Updated: Severe leak from ceiling',
        'root_cause': 'Pipe burst'
    }, format='json')
...
def test_add_photo_to_damage(api_client, admin_user, damage_report):
    ...
    res = api_client.post(f'/api/v1/damage-reports/{damage_report.id}/add-photo/', {
        'image': upload,
        'notes': 'Ceiling water stain'
    }, format='multipart')
...
def test_assign_damage_to_user(api_client, admin_user, assignee_user, damage_report):
    ...
    res = api_client.post(f'/api/v1/damage-reports/{damage_report.id}/assign/', {
...
def test_assess_damage(api_client, admin_user, damage_report):
    ...
    res = api_client.post(f'/api/v1/damage-reports/{damage_report.id}/assess/', {
        'severity': 'critical',
...
def test_resolve_damage(api_client, admin_user, damage_report):
    ...
    res = api_client.post(f'/api/v1/damage-reports/{damage_report.id}/resolve/', {
    ...
    notification = Notification.objects.filter(
        user=damage_report.reported_by,
...
def test_convert_damage_to_change_order(api_client, admin_user, damage_report):
    ...
    res = api_client.post(f'/api/v1/damage-reports/{damage_report.id}/convert-to-co/', {
        'co_title': 'CO: Water Damage Repair',
        'co_description': 'Full bathroom ceiling repair'
    }, format='json')
...
def test_damage_analytics(api_client, admin_user, project):
    ...
    DamageReport.objects.create(project=project, title='D1', severity='low', status='open', category='cosmetic', reported_by=admin_user)
    DamageReport.objects.create(project=project, title='D2', severity='high', status='in_progress', category='structural', reported_by=admin_user)
    DamageReport.objects.create(project=project, title='D3', severity='critical', status='resolved', category='plumbing', reported_by=admin_user)
...
def test_complete_damage_lifecycle(api_client, admin_user, assignee_user, project):
    ...
    # 1. Create damage
    create_res = api_client.post('/api/v1/damage-reports/', {
        'project': project.id,
    ...
    # 2. Assign to user
    assign_res = api_client.post(f'/api/v1/damage-reports/{damage_id}/assign/', {
        'assigned_to': assignee_user.id
    }, format='json')
    assert assign_res.status_code == 200
    
    # 3. Assess severity
    assess_res = api_client.post(f'/api/v1/damage-reports/{damage_id}/assess/', {
        'severity': 'high',
    ...
    assert damage.auto_task.status == 'Completado'
