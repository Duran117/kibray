...
def test_material_request_receive_flow_validation(api_client, user, project):
    ...
    MaterialRequestItem.objects.create(
        request=mr, category="paint", brand="sherwin_williams", product_name="Paint Gallon", quantity=10, unit="gal"
    )

    # Partial receive
    rec1 = api_client.post(
        f"/api/v1/material-requests/{mr.id}/receive/", {"quantity": 5, "notes": "First shipment"}, format="json"
    )
    ...
    # Complete receive
    rec2 = api_client.post(
        f"/api/v1/material-requests/{mr.id}/receive/", {"quantity": 5, "notes": "Final shipment"}, format="json"
    )
...
def test_material_request_legacy_status_mapping(api_client, user, project):
    ...
    mr = MaterialRequest.objects.create(project=project, requested_by=user, status="pending")
    MaterialRequestItem.objects.create(
        request=mr, category="primer", brand="sherwin_williams", product_name="Primer", quantity=20, unit="liter"
    )
