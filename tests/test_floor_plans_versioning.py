...
from core.models import Project, FloorPlan, PlanPin, ClientProjectAccess, PlanPinAttachment
...
def test_create_floor_plan(api_client, admin_user, project):
    ...
    res = api_client.post('/api/v1/floor-plans/', {
        'project': project.id,
        'name': 'First Floor',
        'level': 1,
        'level_identifier': 'Level 1',
        'image': upload
    }, format='multipart')
...
def test_update_floor_plan(api_client, admin_user, floor_plan):
    ...
    res = api_client.patch(f'/api/v1/floor-plans/{floor_plan.id}/', {
        'name': 'Updated Ground Floor',
        'level_identifier': 'Ground'
    }, format='json')
...
def test_create_new_version(api_client, admin_user, floor_plan):
    ...
    res = api_client.post(f'/api/v1/floor-plans/{floor_plan.id}/create-version/', {
        'image': upload
    }, format='multipart')
...
def test_get_migratable_pins(api_client, admin_user, floor_plan):
    ...
    new_version_res = api_client.post(f'/api/v1/floor-plans/{floor_plan.id}/create-version/', {
        'image': upload
    }, format='multipart')
...
def test_migrate_pins(api_client, admin_user, floor_plan):
    ...
    migrate_res = api_client.post(f'/api/v1/floor-plans/{new_plan_id}/migrate-pins/', {
        'pin_mappings': [
...
def test_create_pin(api_client, admin_user, floor_plan):
    ...
    res = api_client.post('/api/v1/plan-pins/', {
        'plan': floor_plan.id,
        'x': '0.45',
        'y': '0.67',
        'title': 'New Pin',
...
def test_update_pin(api_client, admin_user, floor_plan):
    ...
    res = api_client.patch(f'/api/v1/plan-pins/{pin.id}/', {
        'title': 'Updated Pin',
...
def test_add_comment_to_pin(api_client, admin_user, floor_plan):
    ...
    res = api_client.post(f'/api/v1/plan-pins/{pin.id}/comment/', {
...
def test_add_multiple_comments(api_client, admin_user, regular_user, floor_plan, project_access):
    ...
    # Admin comment
    api_client.post(f'/api/v1/plan-pins/{pin.id}/comment/', {
...
def test_update_annotations(api_client, admin_user, floor_plan):
    ...
    # Update annotations
    res = api_client.post(f'/api/v1/plan-pins/{pin.id}/update-annotations/', {
...
def test_filter_pins_by_type(api_client, admin_user, floor_plan):
    ...
    PlanPin.objects.create(plan=floor_plan, x=Decimal('0.2'), y=Decimal('0.2'), pin_type='touchup', created_by=admin_user)
    PlanPin.objects.create(plan=floor_plan, x=Decimal('0.3'), y=Decimal('0.3'), pin_type='damage', created_by=admin_user)
...
def test_filter_pins_by_status(api_client, admin_user, floor_plan):
    ...
    PlanPin.objects.create(plan=floor_plan, x=Decimal('0.2'), y=Decimal('0.2'), status='archived', created_by=admin_user)
...
def test_non_staff_sees_only_accessible_pins(api_client, regular_user, project, project_access, floor_plan):
    ...
    accessible_pin = PlanPin.objects.create(
        plan=floor_plan,
        x=Decimal('0.5'),
        y=Decimal('0.5'),
        title='Accessible'
