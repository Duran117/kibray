...
class TestActivityToTaskConversion:
    ...
    def test_convert_multiple_activities(self):
        ...
        activity1 = PlannedActivity.objects.create(daily_plan=plan, title="Prep walls", status="PENDING")
        activity2 = PlannedActivity.objects.create(daily_plan=plan, title="Paint walls", status="PENDING")
    ...
    def test_skip_completed_activities(self):
        ...
        activity1 = PlannedActivity.objects.create(daily_plan=plan, title="Already done", status="COMPLETED")
        activity2 = PlannedActivity.objects.create(daily_plan=plan, title="Not done", status="PENDING")
    ...
    def test_convert_activity_with_employee_assignment(self):
        ...
        employee = Employee.objects.create(user=user, hire_date=date.today(), hourly_rate=Decimal("20.00"))

        activity = PlannedActivity.objects.create(daily_plan=plan, title="Paint walls", status="PENDING")
    ...
    def test_convert_activity_with_schedule_link(self):
        ...
        activity = PlannedActivity.objects.create(
            daily_plan=plan, title="Paint day 1", schedule_item=schedule_item, status="PENDING"
        )
...
class TestMaterialConsumption:
    ...
    def test_auto_consume_materials_basic(self):
        ...
        location = InventoryLocation.objects.create(name="Project Site", project=project)

        item = InventoryItem.objects.create(name="Tape", sku="TAPE001", category="MATERIAL", unit="rolls")

        # Create ProjectInventory with stock
        ProjectInventory.objects.create(item=item, location=location, quantity=Decimal("100"))
    ...
    def test_auto_consume_multiple_materials(self):
        ...
        tape = InventoryItem.objects.create(name="Tape", sku="TAPE001", category="MATERIAL", unit="rolls")

        paint = InventoryItem.objects.create(name="Paint - White", sku="PAINT001", category="PINTURA", unit="gallons")

        # Create ProjectInventory for both items
        ProjectInventory.objects.create(item=tape, location=location, quantity=Decimal("100"))
        ProjectInventory.objects.create(item=paint, location=location, quantity=Decimal("50"))

        consumption_data = {"Tape": 10, "Paint": 2}
        movements = plan.auto_consume_materials(consumption_data, user)
...
class TestPlannedActivityTracking:
    ...
    def test_activity_time_variance_under_budget(self):
        ...
        activity = PlannedActivity.objects.create(
            daily_plan=plan, title="Paint walls", estimated_hours=Decimal("8.00"), actual_hours=Decimal("6.00")
        )
    ...
    def test_activity_time_variance_over_budget(self):
        ...
        activity = PlannedActivity.objects.create(
            daily_plan=plan, title="Paint walls", estimated_hours=Decimal("8.00"), actual_hours=Decimal("10.00")
        )
    ...
    def test_activity_time_variance_null_when_no_data(self):
        ...
        activity = PlannedActivity.objects.create(daily_plan=plan, title="Paint walls")
