...
from core.models import InventoryItem, InventoryLocation, InventoryMovement, Project, ProjectInventory
...
def test_inventory_item_avg_cost_calculation():
    ...
    item = InventoryItem.objects.create(
        name="Paint Gallon", category="PINTURA", valuation_method="AVG", average_cost=Decimal("0")
    )
...
def test_inventory_fifo_cost_calculation():
    ...
    item = InventoryItem.objects.create(name="Brush Set", category="HERRAMIENTA", valuation_method="FIFO")
...
def test_inventory_transfer_multi_location():
    ...
    item = InventoryItem.objects.create(
        name="Roller", category="HERRAMIENTA", valuation_method="AVG", average_cost=Decimal("15")
    )

    warehouse_stock = ProjectInventory.objects.create(item=item, location=warehouse, quantity=Decimal("50"))
...
def test_inventory_negative_prevention():
    ...
    item = InventoryItem.objects.create(name="Tape", category="MATERIAL", valuation_method="AVG")

    # Only 10 units available
    ProjectInventory.objects.create(item=item, location=warehouse, quantity=Decimal("10"))
...
def test_inventory_stock_adjustment():
    ...
    item = InventoryItem.objects.create(name="Ladder", category="ESCALERA", is_equipment=True)

    # Initial stock: 5 units
    stock = ProjectInventory.objects.create(item=item, location=warehouse, quantity=Decimal("5"))
...
def test_low_stock_detection():
    ...
    item = InventoryItem.objects.create(name="Brush", category="HERRAMIENTA", low_stock_threshold=Decimal("20"))

    warehouse = InventoryLocation.objects.create(name="Main", is_storage=True)

    # Stock below threshold
    ProjectInventory.objects.create(item=item, location=warehouse, quantity=Decimal("15"))
...
def test_inventory_valuation_methods():
    ...
    fifo_item = InventoryItem.objects.create(
        name="FIFO Item", category="MATERIAL", valuation_method="FIFO", average_cost=Decimal("10")
    )

    # AVG item
    avg_item = InventoryItem.objects.create(
        name="AVG Item", category="MATERIAL", valuation_method="AVG", average_cost=Decimal("15")
    )

    # LIFO item
    lifo_item = InventoryItem.objects.create(
        name="LIFO Item", category="MATERIAL", valuation_method="LIFO", average_cost=Decimal("20")
    )
