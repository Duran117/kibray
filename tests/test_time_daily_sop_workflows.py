...
class TestTimeTrackingWorkflow:
    ...
    def test_timeentry_create_post_minimal(self):
        ...
        emp = Employee.objects.create(
            user=self.admin_user, first_name="A", last_name="B", social_security_number="123", hourly_rate=10
        )
    ...
    def test_timeentry_edit_delete_cycle(self):
        ...
        emp = Employee.objects.create(
            user=self.admin_user, first_name="C", last_name="D", social_security_number="456", hourly_rate=12
        )
        project = Project.objects.create(name="EditDel Project", start_date=timezone.now().date())
        entry = TimeEntry.objects.create(
            employee=emp, project=project, date=timezone.now().date(), start_time=timezone.now().time()
        )
        # Edit view
