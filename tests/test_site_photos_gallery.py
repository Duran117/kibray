...
from core.models import ClientProjectAccess, DamageReport, Project, SitePhoto
...
def admin_user(db):
    return User.objects.create_user(
        username="admin_photo", password="pass123", email="admin@example.com", is_staff=True
    )
...
def regular_user(db):
    return User.objects.create_user(
        username="client_user", password="pass123", email="client@example.com", is_staff=False
    )
...
def project(db):
    return Project.objects.create(
        name="GalleryProject", client="ACME Corp", start_date=date.today(), address="123 Construction St"
    )
...
def project_access(db, regular_user, project):
    ...
    return ClientProjectAccess.objects.create(
        user=regular_user, project=project, role="client", can_comment=True, can_create_tasks=False
    )
...
def test_site_photo_update_metadata(api_client, admin_user, project):
    ...
    create_res = api_client.post(
        "/api/v1/site-photos/",
        {"project": project.id, "image": upload, "caption": "Original caption"},
        format="multipart",
    )
    ...
    update_res = api_client.patch(
        f"/api/v1/site-photos/{photo_id}/",
        {"caption": "Updated caption", "notes": "Added some notes", "room": "Kitchen"},
        format="json",
    )
...
def test_site_photo_delete(api_client, admin_user, project):
    ...
    create_res = api_client.post(
        "/api/v1/site-photos/", {"project": project.id, "image": upload, "caption": "To be deleted"}, format="multipart"
    )
...
def test_filter_by_project(api_client, admin_user):
    ...
    for i in range(3):
        ...
        api_client.post(
            "/api/v1/site-photos/",
            {"project": proj1.id, "image": upload1, "caption": f"Project 1 Photo {i}"},
            format="multipart",
        )
    ...
    for i in range(2):
        ...
        api_client.post(
            "/api/v1/site-photos/",
            {"project": proj2.id, "image": upload2, "caption": f"Project 2 Photo {i}"},
            format="multipart",
        )
...
def test_filter_by_photo_type(api_client, admin_user, project):
    ...
    for ptype in photo_types:
        ...
        api_client.post(
            "/api/v1/site-photos/",
            {"project": project.id, "image": upload, "caption": f"{ptype} photo", "photo_type": ptype},
            format="multipart",
        )
...
def test_filter_by_date_range(api_client, admin_user, project):
    ...
    api_client.post(
        "/api/v1/site-photos/", {"project": project.id, "image": upload, "caption": "Recent photo"}, format="multipart"
    )
...
def test_search_by_caption(api_client, admin_user, project):
    ...
    api_client.post(
        "/api/v1/site-photos/",
        {"project": project.id, "image": upload1, "caption": "Kitchen renovation progress"},
        format="multipart",
    )
    ...
    api_client.post(
        "/api/v1/site-photos/",
        {"project": project.id, "image": upload2, "caption": "Bathroom tile installation"},
        format="multipart",
    )
...
def test_gallery_action(api_client, admin_user, project):
    ...
    for ptype in photo_types:
        for i in range(2):  # 2 photos per type
            ...
            api_client.post(
                "/api/v1/site-photos/",
                {"project": project.id, "image": upload, "caption": f"{ptype} photo {i}", "photo_type": ptype},
                format="multipart",
            )
...
def test_non_staff_sees_only_accessible_projects(api_client, regular_user, project, project_access):
    ...
    other_project = Project.objects.create(name="RestrictedProject", client="Other Client", start_date=date.today())
...
def test_pagination_enabled(api_client, admin_user, project):
    ...
    for i in range(30):
        ...
        api_client.post(
            "/api/v1/site-photos/",
            {"project": project.id, "image": upload, "caption": f"Photo {i}"},
            format="multipart",
        )
...
def test_thumbnail_auto_generation(api_client, admin_user, project):
    ...
    res = api_client.post(
        "/api/v1/site-photos/",
        {"project": project.id, "image": upload, "caption": "Thumbnail test"},
        format="multipart",
    )
...
def test_upload_without_project_fails(api_client, admin_user):
    ...
    res = api_client.post("/api/v1/site-photos/", {"image": upload, "caption": "No project"}, format="multipart")
