...
class Test2FATOTPAPI:
    def test_setup_enable_and_login_with_otp(self, api_client, user, monkeypatch):
        ...
        res_login_no_otp = api_client.post('/api/v1/auth/login/', {
            'username': 'twofa', 'password': 'pass123'
        }, format='json')
        ...
        res_login = api_client.post('/api/v1/auth/login/', {
            'username': 'twofa', 'password': 'pass123', 'otp': otp
        }, format='json')
