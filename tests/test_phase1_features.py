...
from core.models import DailyLog, Project, Task, TaskStatusChange, TaskTemplate, WeatherSnapshot
...
def admin_user(db):
    ...
    return User.objects.create_user(
        username="admin_phase1", email="admin@phase1.com", password="admin123", is_staff=True, is_superuser=True
    )
...
def regular_user(db):
    ...
    return User.objects.create_user(username="user_phase1", email="user@phase1.com", password="user123")
...
def project(db):
    ...
    return Project.objects.create(
        name="Test Project Phase 1", client="Test Client", start_date=date.today(), budget_total=Decimal("15000.00")
    )
...
def daily_log(db, project, admin_user):
    ...
    return DailyLog.objects.create(
        project=project, date=date.today(), weather="Soleado", crew_count=5, created_by=admin_user
    )
...
class TestDailyLogPlanning:
    ...
    def test_instantiate_planned_templates(self, daily_log, task_template, admin_user):
        ...
        # Instantiate
        created_tasks = daily_log.instantiate_planned_templates(created_by=admin_user, assigned_to=None)
...
class TestWeatherSnapshot:
    ...
    def test_is_stale_fresh_snapshot(self, project):
        ...
        snap = WeatherSnapshot.objects.create(
            project=project, date=date.today(), source="openweathermap", temperature_max=Decimal("25.0")
        )
    ...
    def test_unique_constraint(self, project):
        ...
        with pytest.raises(Exception):  # Django IntegrityError
            WeatherSnapshot.objects.create(
                project=project, date=date.today(), source="openweathermap", temperature_max=Decimal("26.0")
            )
...
class TestWeatherServiceIntegration:
    ...
    def test_get_or_create_snapshot_creates_new(self, mock_get_weather, project):
        ...
        snap = get_or_create_snapshot(project=project, latitude=29.4241, longitude=-98.4936, date=timezone.now())
...
class TestTaskTemplate:
    ...
    def test_create_task_from_template(self, task_template, project, admin_user):
        ...
        task = task_template.create_task(project=project, created_by=admin_user, assigned_to=None)
