...
from core.models import Project, DailyPlan, PlannedActivity, Employee, Task
...
@pytest.fixture
def project(db):
    return Project.objects.create(name='DailyPlan Project', client='Acme', start_date=date.today(), address='123 Main St, City')
...
class TestDailyPlanActions:
    ...
    def test_productivity_score(self, api_client, user, project):
        api_client.force_authenticate(user=user)
        plan = DailyPlan.objects.create(project=project, plan_date=date.today()+timedelta(days=1), completion_deadline=date.today(), estimated_hours_total=8.0, actual_hours_worked=6.0)
