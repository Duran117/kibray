...
from core.models import Project, InventoryLocation, ProjectInventory, InventoryItem, Expense
...
@pytest.fixture
def project(db):
    return Project.objects.create(name='Materials Project', client='ACME', start_date=date.today(), address='123 Main St, City')
...
def test_material_request_flow_receive_partial_and_full(api_client, user, project):
    ...
    mr_payload = {
        ...
        'notes': 'Test MR',
        'items': [
            {'category': 'tape', 'brand': '3m', 'brand_text': '3M', 'product_name': 'Masking Tape', 'quantity': '10', 'unit': 'roll'},
            {'category': 'paint', 'brand': 'sherwin_williams', 'brand_text': 'Sherwin‑Williams', 'product_name': 'Emerald Interior', 'quantity': '5', 'unit': 'gal'}
        ]
    }
    ...
    recv_res = api_client.post(f'/api/v1/material-requests/{mr_id}/receive/', {
    ...
    recv_all = api_client.post(f'/api/v1/material-requests/{mr_id}/receive/', {
        'items': [
    ...
    recv2 = api_client.post(f'/api/v1/material-requests/{mr_id}/receive/', {
...
def test_direct_purchase_expense_creates_inventory(api_client, user, project):
    ...
    mr_payload = {
        ...
        'items': [
            {'category': 'paint', 'brand': 'sherwin_williams', 'brand_text': 'SW', 'product_name': 'Emerald', 'quantity': '2', 'unit': 'gal'}
        ]
    }
    res = api_client.post('/api/v1/material-requests/', mr_payload, format='json')
    assert res.status_code in (200, 201)
    ...
    dp = api_client.post(f'/api/v1/material-requests/{mr_id}/direct_purchase_expense/', {
        'total_amount': '250.00'
    }, format='json')
...
def test_catalog_create_and_filter(api_client, user, project):
    ...
    res1 = api_client.post('/api/v1/material-catalog/', {
        'project': None,
        'category': 'tape',
        'brand_text': '3M',
    ...
    res2 = api_client.post('/api/v1/material-catalog/', {
        'project': project.id,
        'category': 'paint',
        'brand_text': 'SW',
...
def test_inventory_negative_issue_prevented(api_client, user, project):
    api_client.force_authenticate(user=user)
    # Create item
    item_res = api_client.post('/api/v1/inventory/items/', {
        'name': 'Sandpaper', 'category': 'MATERIAL', 'unit': 'pcs'
    }, format='json')
    assert item_res.status_code in (200, 201)
    item_id = item_res.data['id']
    # Create project location
    loc_res = api_client.post('/api/v1/inventory/locations/', {
        'name': 'Site A', 'project': project.id, 'is_storage': False
    }, format='json')
    assert loc_res.status_code in (200, 201)
    loc_id = loc_res.data['id']
    # Attempt issue more than available (0)
    move_res = api_client.post('/api/v1/inventory/movements/', {
        'item': item_id,
