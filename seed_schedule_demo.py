...
cat1, _ = ScheduleCategory.objects.get_or_create(project=project, name="Preparación", defaults={"order": 1})

cat2, _ = ScheduleCategory.objects.get_or_create(project=project, name="Construcción", defaults={"order": 2})

cat3, _ = ScheduleCategory.objects.get_or_create(project=project, name="Acabados", defaults={"order": 3})
...
for item_data in items_data:
    item, created = ScheduleItem.objects.get_or_create(project=project, title=item_data["title"], defaults=item_data)
    if created:
