...
def quick_planner_entry(request):
    ...
    if not (request.user.is_staff or request.user.is_superuser):
        return JsonResponse(
            {"error": gettext("This feature is only available for Admin/PM users.")}, status=403
        )
...
def strategic_ritual_wizard(request):
    ...
    existing_ritual = DailyRitualSession.objects.filter(user=request.user, date=today).first()

    context = {"existing_ritual": existing_ritual, "has_ritual_today": existing_ritual is not None}
...
def get_active_habits(request):
    ...
    """
    habits = ExecutiveHabit.objects.filter(user=request.user, is_active=True).values(
        "id", "title", "frequency"
    )
...
def get_random_vision(request):
    ...
    """
    vision = LifeVision.objects.filter(user=request.user).order_by("?").first()
...
def complete_ritual(request):
    ...
    try:
        ...
        for habit_id in habit_ids:
            try:
                ...
                HabitCompletion.objects.get_or_create(
                    habit=habit, completed_date=today, defaults={"session": ritual}
                )
        ...
        return JsonResponse(
            {"success": False, "error": gettext("%(error)s") % {"error": str(e)}}, status=400
        )
    except Exception as e:
        return JsonResponse(
            {"success": False, "error": gettext("Unexpected error: %(error)s") % {"error": str(e)}},
            status=500,
        )
...
def planner_calendar_feed(request, user_token):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{user.username}_strategic_planner.ics"'
    )
    return response
...
def today_ritual_summary(request):
    ...
    ritual = DailyRitualSession.objects.filter(user=request.user, date=today).first()
        ...
        return JsonResponse(
            {
                ...
                "frog": {
                    ...
                    "scheduled_start": frog.scheduled_start.isoformat()
                    if frog.scheduled_start
                    else None,
    ...
    focus_session = DailyFocusSession.objects.filter(user=request.user, date=today).first()
        ...
        return JsonResponse(
            {
                ...
                "frog": {
                    ...
                    "micro_steps_progress": 100
                    if frog_task.is_completed
                    else 0,  # FocusTask might not have progress calc
                    "scheduled_start": frog_task.scheduled_start.isoformat()
                    if frog_task.scheduled_start
                    else None,
                    "scheduled_end": frog_task.scheduled_end.isoformat()
                    if frog_task.scheduled_end
                    else None,
    ...
    return JsonResponse({"has_ritual": False, "message": "No ritual completed today"})
...
def toggle_power_action_status(request, action_id):
    ...
    try:
        ...
        action.save()
        return JsonResponse(
            {"success": True, "new_status": action.status, "is_completed": action.status == "DONE"}
        )
...
def update_micro_step(request, action_id, step_index):
    ...
    try:
        ...
        action.micro_steps[step_index]["done"] = not action.micro_steps[step_index].get(
            "done", False
        )
        action.save()
        return JsonResponse(
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "step_done": task.checklist[step_index]["done"], "progress": progress}
        )
    except FocusTask.DoesNotExist:
        return JsonResponse({"error": "Action not found"}, status=404)
...
def planner_stats(request):
    ...
    rituals_completed = DailyRitualSession.objects.filter(
        user=request.user, date__in=last_7_days, completed_at__isnull=False
    ).count()

    # Frog completion rate (last 30 days)
    last_30_days = timezone.now().date() - timedelta(days=30)
    total_frogs = PowerAction.objects.filter(
        session__user=request.user, session__date__gte=last_30_days, is_frog=True
    ).count()

    completed_frogs = PowerAction.objects.filter(
        session__user=request.user, session__date__gte=last_30_days, is_frog=True, status="DONE"
    ).count()
    ...
    for habit in habits:
        # Get last completion
        last_completion = (
            HabitCompletion.objects.filter(habit=habit).order_by("-completed_date").first()
        )
        ...
        completions_this_month = HabitCompletion.objects.filter(
            habit=habit, completed_date__gte=this_month_start
        ).count()
        ...
        habit_stats.append(
            {
                ...
                "last_completion": last_completion.completed_date.isoformat()
                if last_completion
                else None,
...
def ai_process_brain_dump(request):
    ...
    try:
        ...
        if not text or not text.strip():
            return JsonResponse({"error": "Brain dump text is required"}, status=400)

        # Get user context
        visions = list(
            LifeVision.objects.filter(user=request.user).values_list("title", flat=True)[:5]
        )

        user_context = {"visions": visions, "energy_level": data.get("energy_level", 5)}
        ...
        return JsonResponse({"error": f"Error processing brain dump: {str(e)}"}, status=500)
...
def ai_suggest_frog(request):
    ...
    try:
        ...
        if not items:
            return JsonResponse({"error": "No high-impact items provided"}, status=400)

        user_context = {"energy_level": data.get("energy_level", 5)}
        ...
        return JsonResponse({"error": f"Error suggesting frog: {str(e)}"}, status=500)
...
def ai_generate_micro_steps(request):
    ...
    try:
        ...
        if not frog_title or not frog_title.strip():
            return JsonResponse({"error": "Frog title is required"}, status=400)
        ...
        return JsonResponse({"micro_steps": micro_steps})

    except Exception as e:
        return JsonResponse({"error": f"Error generating micro-steps: {str(e)}"}, status=500)
...
def ai_suggest_time_blocks(request):
    ...
    try:
        ...
        return JsonResponse({"error": f"Error suggesting time blocks: {str(e)}"}, status=500)
