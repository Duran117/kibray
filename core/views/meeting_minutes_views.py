...
def project_minute_create(request, project_id):
    ...
    if not (request.user.is_staff or request.user.is_superuser):
        messages.error(request, _("You don't have permission to create meeting minutes."))
    ...
    if request.method == "POST":
        ...
        if not title or not event_date_str:
            messages.error(request, _("Title and date are required."))
        else:
            try:
            ...
            messages.success(request, _("Meeting minutes created successfully."))
            return redirect("project_minutes_list", project_id=project.id)
...
def project_minute_detail(request, minute_id):
    ...
    if not (request.user.is_staff or request.user.is_superuser or minute.visible_to_client):
        messages.error(request, _("You don't have permission to view these meeting minutes."))
...
def project_minute_edit(request, minute_id):
    ...
    if not (request.user.is_staff or request.user.is_superuser):
        messages.error(request, _("You don't have permission to edit meeting minutes."))
    ...
    if request.method == "POST":
        ...
        minute.save()
        messages.success(request, _("Meeting minutes updated successfully."))
        return redirect("project_minute_detail", minute_id=minute.id)
...
def project_minute_delete(request, minute_id):
    ...
    if not (request.user.is_staff or request.user.is_superuser):
        messages.error(request, _("You don't have permission to delete meeting minutes."))
        return redirect("project_minute_detail", minute_id=minute.id)

    if request.method == "POST":
        minute.delete()
        messages.success(request, _("Meeting minutes deleted successfully."))
