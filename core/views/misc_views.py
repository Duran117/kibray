...
def color_sample_client_signature_view(request, sample_id, token=None):
    ...
    if token is not None:
        # Two valid token formats:
        #  1) The raw secrets.token_urlsafe value stored on the model
        #     (generated by ColorSample.generate_signature_token()).
        #  2) A django.core.signing.dumps payload (legacy / future-proof).
        # Try (1) first with constant-time compare; fall back to (2).
        import hmac as _hmac
        stored = color_sample.signature_token or ""
        token_ok = bool(stored) and _hmac.compare_digest(str(token), str(stored))
        if not token_ok:
            try:
