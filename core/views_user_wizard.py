...
def user_list_view(request):
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        html = render_to_string(
            "core/wizards/partials/user_list_rows.html", context, request=request
        )
...
def validate_user_step(data, user_obj=None):
    ...
    if step == 1:  # Basic Info
        ...
        if not username:
            ...
            .exclude(pk=user_obj.pk if user_obj else None)
            .exists()
        ):
        ...
        if not email:
            ...
            User.objects.filter(email=email).exclude(pk=user_obj.pk if user_obj else None).exists()
        ):
