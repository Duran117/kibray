...
def dashboard_client(request):
    ...
    for project in projects:
        ...
        # Intentar encontrar item futuro primero
    ...
    # Support for ?project_id=X to select a specific project on load
    # This is used when navigating back from a function to remember which project was active
    selected_project_id = request.GET.get("project_id")
    active_project_index = 0  # Default to first project
    
    if selected_project_id:
        try:
            selected_id = int(selected_project_id)
            for idx, data in enumerate(project_data):
                if data["project"].id == selected_id:
...
def client_project_view(request, project_id):
    ...
    # === DOCUMENTOS PÚBLICOS PARA CLIENTE ===
    from core.models import ProjectFile, FileCategory
    
    # Documentos marcados como públicos (is_public=True)
    public_files = ProjectFile.objects.filter(
        project=project,
        is_public=True
    ).select_related("category", "uploaded_by").order_by("-uploaded_at")[:20]
    
    # Categorías de archivos para organizar
    file_categories_with_public = FileCategory.objects.filter(
        project=project,
    ...
    context = {
        ...
        "file_categories_with_public": file_categories_with_public,
    }
