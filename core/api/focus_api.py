...
class DailyFocusSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return DailyFocusSession.objects.filter(
            user=self.request.user
        ).prefetch_related('focus_tasks').order_by('-date')
    ...
    def today(self, request):
        ...
        session = DailyFocusSession.objects.filter(
            user=request.user,
            date=today
        ).prefetch_related('focus_tasks').first()
        ...
        return Response(
            {'detail': 'No session for today yet.'},
    ...
    def this_week(self, request):
        ...
        sessions = DailyFocusSession.objects.filter(
            user=request.user,
...
class FocusTaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return FocusTask.objects.filter(
            session__user=self.request.user
        ).select_related('session').order_by('-session__date', 'order')
    ...
    def upcoming(self, request):
        ...
        tasks = FocusTask.objects.filter(
            session__user=request.user,
            scheduled_start__gte=now,
            scheduled_start__lte=week_from_now,
            is_completed=False
        ).select_related('session').order_by('scheduled_start')
...
from django.db import models  # For aggregation
