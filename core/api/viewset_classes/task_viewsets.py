...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def touchup_board(self, request):
        ...
        assigned_to_me = request.query_params.get("assigned_to_me", "").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        return Response(
            {
                "columns": columns,
                "totals": totals,
                "filters": {"project": project_id, "assigned_to_me": assigned_to_me},
            }
        )
    ...
    def by_status(self, request):
        ...
        if not status_param:
            return Response(
                {"error": "status parameter is required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def update_status(self, request, pk=None):
        ...
        if new_status == "Completada" and task.is_touchup:
            ...
            if not has_image:
                return Response(
                    {"error": "Touch-up completion requires a photo"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def add_dependency(self, request, pk=None):
        ...
        if not dep_id:
            return Response(
                {"error": "dependency_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            dep_task = Task.objects.get(pk=dep_id)
        except Task.DoesNotExist:
            return Response(
                {"error": "Dependency task not found"}, status=status.HTTP_404_NOT_FOUND
            )
        if dep_task == task:
            return Response(
                {"error": "Task cannot depend on itself"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def start_tracking(self, request, pk=None):
        ...
        if not task.can_start():
            return Response(
                {"error": "Dependencies not completed"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {
                ...
                if hasattr(task, "get_time_tracked_hours")
                else 0,
            },
    ...
    def add_image(self, request, pk=None):
        ...
        return Response(
            {"version": new_image.version, "image_id": new_image.id}, status=status.HTTP_200_OK
        )
