...
from core.models import ChangeOrder, Employee, Project, Task
...
class AnalyticsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        avg_completion = completed_tasks / total_tasks * 100 if total_tasks > 0 else 0

        # Calculate total revenue
        total_revenue = Project.objects.aggregate(total=Sum("total_income"))["total"] or Decimal(
            "0.00"
        )
        ...
        # Budget Chart Data
        budget_data = Project.objects.filter(created_at__gte=date_from).aggregate(
            budgeted=Sum("budget_total"), actual=Sum("total_expenses")
        )
    ...
    def project_analytics(self, request):
        ...
        if not project_id:
            return Response(
                {"error": "project_id parameter is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        changeorders_summary = {
            ...
            "total_amount": float(
                project.change_orders.aggregate(total=Sum("amount"))["total"] or 0
            ),
        }
