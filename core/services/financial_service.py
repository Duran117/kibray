...
class FinancialAnalyticsService:
    ...
    def get_cash_flow_projection(self, days: int = 30) -> Dict[str, Any]:
        ...
        for inv in invoices:
            ...
            income_by_week[wk] = income_by_week.get(wk, Decimal("0")) + (inv["amount"] or Decimal("0"))

        four_weeks_ago = self.as_of - timedelta(weeks=4)
        payroll = PayrollRecord.objects.filter(week_start__gte=four_weeks_ago, week_end__lte=self.as_of)
    ...
    def get_project_margins(self) -> list[Dict[str, Any]]:
        ...
        for p in Project.objects.filter(is_archived=False).order_by("name"):
            ...
            labor_cost = (
                p.timeentry_set.aggregate(t=Sum(F("hours_worked") * F("employee__hourly_rate")))["t"]
                or Decimal("0")
            )
