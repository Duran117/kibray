...
def notification_badges(request):
    ...
    try:
        from core.models import TimeEntry, MaterialRequest, Issue, ChangeOrder, Task, ColorSample, Notification
        ...
        badges['unassigned_time_count'] = TimeEntry.objects.filter(
            project__isnull=True
        ).count()
        
        # Pending material requests
        badges['pending_materials_count'] = MaterialRequest.objects.filter(
            status='pending'
        ).count()
        ...
        if request.user.is_staff:
            ...
            badges['new_client_tasks_count'] = Task.objects.filter(
                status='Pendiente',
                created_by__profile__role='client'
            ).count()
