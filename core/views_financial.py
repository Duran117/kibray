...
from django.db.models import Sum, Avg, Q, F, Count, ExpressionWrapper, FloatField, DecimalField
...
from .models import (
    Project, Invoice, Expense, Income, TimeEntry, ChangeOrder,
    Employee, EmployeePerformanceMetric
)
...
def financial_dashboard(request):
    ...
    overdue_invoices = Invoice.objects.filter(
        status__in=['sent', 'viewed', 'approved'],
        date_issued__lt=overdue_threshold
    )
...
def productivity_dashboard(request):
    ...
    billable_hours = time_entries.filter(
        change_order__isnull=False  # Assigned to a CO
    ).aggregate(
        total=Coalesce(Sum('hours_worked'), Decimal('0.00'))
    )['total']
    ...
    ).filter(total_hours__gt=0).order_by('-productivity')
    ...
    productivity_by_week = time_entries.extra(
        select={'week': 'strftime("%%W", date)'}
...
def export_financial_data(request):
    ...
    if export_type == 'expenses':
        ...
        # Header
        writer.writerow([
            'Date', 'Project', 'Category', 'Description', 
            'Amount', 'Vendor', 'Receipt'
        ])
        ...
        # Header
        writer.writerow([
            'Date', 'Project', 'Amount', 'Payment Method', 
        ...
        # Header
        writer.writerow([
            'Invoice Number', 'Date Issued', 'Date Due', 'Project', 
            'Client', 'Total Amount', 'Status', 'Amount Paid', 'Balance'
        ])
...
def _update_employee_metrics(metric, year):
    ...
    # Hours
    metric.total_hours_worked = time_entries.aggregate(
        Sum('hours_worked')
    )['hours_worked__sum'] or Decimal('0.00')
    
    metric.billable_hours = time_entries.filter(
        change_order__isnull=False
    ).aggregate(
        Sum('hours_worked')
    )['hours_worked__sum'] or Decimal('0.00')
    ...
    if metric.total_hours_worked > 0:
        metric.productivity_rate = (
            metric.billable_hours / metric.total_hours_worked * 100
        )
