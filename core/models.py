...
class ColorSample(models.Model):
    ...
    def get_signature_url(self):
        """Get the public URL for client signature"""
        from django.urls import reverse
        if not self.signature_token:
    ...
    def sign_by_client(self, signature_image, signed_name, ip_address=None):
        ...
        # Notify project team
        self._notify_client_signed(signed_name)
        
        return True

    def _notify_client_signed(self, signed_name):
        """Notify PMs and admins when client signs a color sample"""
        from django.contrib.auth.models import User
        from django.urls import reverse
        from core.models import Notification
        ...
        link = reverse("color_sample_detail", args=[self.id])
        
        for recipient in recipients:
            Notification.objects.create(
                user=recipient,
                notification_type="color_sample_signed",
                title=f"Color aprobado por cliente: {self.name or self.code}",
                message=f"{signed_name} firmó y aprobó el color '{self.name or self.code}' para {self.project.name}",
                related_object_type="color_sample",
                related_object_id=self.id,
                link_url=link,
            )
    ...
    def needs_client_signature(self):
        """Check if this sample requires and is waiting for client signature"""
...
class DailyPlan(models.Model):
    ...
    # ===== AI ENHANCEMENT METHODS (Dec 2025) =====

    def run_ai_analysis(self):
        """
        Trigger comprehensive AI analysis of this plan

        Returns:
            AnalysisReport object with all findings
        """
        from core.services.daily_plan_ai import daily_plan_ai

        report = daily_plan_ai.analyze_plan(self)

        # Store suggestions
        for material_issue in report.material_issues:
            AISuggestion.objects.create(
                daily_plan=self,
                suggestion_type="missing_material",
                severity=material_issue.severity,
                title=f"Material Issue: {material_issue.material_name}",
                description=f"Required: {material_issue.required} {material_issue.unit}, Available: {material_issue.available} {material_issue.unit}",
                suggested_action=material_issue.suggestion,
                auto_fixable=material_issue.auto_fixable,
            )

        for employee_issue in report.employee_issues:
            AISuggestion.objects.create(
                daily_plan=self,
                suggestion_type="employee_conflict",
                severity=employee_issue.severity,
                title=f"Employee Issue: {employee_issue.employee_name}",
                description=employee_issue.description,
                suggested_action=employee_issue.suggestion,
                auto_fixable=False,
            )
        ...
        for safety_issue in report.safety_issues:
            AISuggestion.objects.create(
                daily_plan=self,
        ...
        AIAnalysisLog.objects.create(
            daily_plan=self, analysis_type="full", findings=report.to_dict(), auto_actions_taken=[]
        )
    ...
    @property
    def ai_score(self):
        """Get latest AI analysis score (0-100)"""
        latest_log = self.ai_analyses.filter(analysis_type="full").first()
        if latest_log and latest_log.findings:
            return latest_log.findings.get("overall_score")
        return None

    @property
    def pending_suggestions(self):
        """Get pending AI suggestions"""
        return self.ai_suggestions.filter(status="pending")

    @property
    def critical_suggestions(self):
        """Get critical AI suggestions"""
        return self.ai_suggestions.filter(severity="critical", status="pending")
...
# ============================================================================
# DAILY PLAN AI MODELS (Module 12 Enhancement)
# ============================================================================
