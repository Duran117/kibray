...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(settings.MEDIA_ROOT):
            self.stdout.write(self.style.WARNING('   ⚠️  Directory does not exist, creating...'))
            try:
        ...
        if os.access(settings.MEDIA_ROOT, os.W_OK):
            ...
            self.stdout.write(self.style.ERROR('   ❌ Not writable!'))
            return
        ...
        self.stdout.write('\n📝 Next steps:')
        self.stdout.write('   1. Upload new floor plan images')
        self.stdout.write('   2. Old images (< 1KB) are corrupted and need to be replaced')
