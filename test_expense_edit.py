...
from core.models import Project, Expense
...
def test_expense_edit_flow():
    ...
    # Crear gasto inicial
    expense = Expense.objects.create(project=project, amount=Decimal('120.50'), project_name='Compra Material', date=date.today(), category='MATERIALES')
    eid = expense.id
    print(f'✓ Gasto inicial creado ID={eid} monto={expense.amount}')

    # Simular edición vía vista
    resp = client.post(f'/admin-panel/expenses/{eid}/edit/', data={
        'project': project.id,
    ...
    resp_bad = client.post(f'/admin-panel/expenses/{eid}/edit/', data={
        'project': project.id,
        'project_name': 'Compra Material Editada',
        'amount': '-10',
        'date': date.today().strftime('%Y-%m-%d'),
