...
def check_environment_variable():
    ...
    print("✓ Método de carga: os.environ.get('OPENAI_API_KEY', '')")

    if env_key:
        print("✅ Variable encontrada en OS environment")
        ...
        if env_key.startswith("sk-"):
            ...
            print("⚠️  Formato inusual (no comienza con 'sk-')")

    else:
        print("❌ Variable NO encontrada en OS environment")
        return False
    ...
    if hasattr(settings, 'OPENAI_API_KEY'):
        ...
        print("✅ Variable accesible desde settings.OPENAI_API_KEY")
        print(f"   Longitud: {len(settings_key)} caracteres")

        if settings_key == env_key:
            print("✅ Coincide con variable de entorno")
        else:
            print("⚠️  NO coincide con variable de entorno")

        if settings_key:
            print("✅ Variable tiene valor (no está vacía)")
        else:
            print("❌ Variable está vacía")
            return False
    else:
        print("❌ settings.OPENAI_API_KEY no está definido")
        return False
    ...
    if hasattr(settings, 'OPENAI_MODEL'):
        ...
        print("⚠️  OPENAI_MODEL no configurado (usará default)")
...
def check_dependencies():
    ...
    try:
        import openai
        print("✅ Librería 'openai' instalada")
        print(f"   Versión: {openai.__version__}")

        # Try to import OpenAI client
        try:
            from openai import OpenAI
            print("✅ Cliente OpenAI importable")
            return True
...
def test_api_connection():
    ...
    try:
        ...
        print("   Endpoint: chat.completions.create")
        print(f"   Modelo: {getattr(settings, 'OPENAI_MODEL', 'gpt-4o-mini')}")
        print("   Mensaje: '¿Qué es 2+2?'")
        ...
        if "authentication" in error_msg or "api key" in error_msg or "unauthorized" in error_msg:
            ...
            print("   - Verificar en: https://platform.openai.com/api-keys")
            print("   - Regenerar key si es necesario")

        elif "rate limit" in error_msg or "quota" in error_msg:
            print("\n🟡 DIAGNÓSTICO: Límite de Rate/Quota")
            print("   - Has excedido el rate limit o quota")
            print("   - Verificar uso en: https://platform.openai.com/usage")
            print("   - Esperar o actualizar plan")

        elif "connection" in error_msg or "network" in error_msg or "timeout" in error_msg:
            print("\n🟠 DIAGNÓSTICO: Error de Conectividad")
            print("   - Problema de red o firewall")
            print("   - Railway puede estar bloqueando")
            print("   - Verificar logs de Railway")

        elif "model" in error_msg:
            print("\n🟡 DIAGNÓSTICO: Error de Modelo")
            print("   - Modelo no disponible o incorrecto")
            print(f"   - Modelo configurado: {getattr(settings, 'OPENAI_MODEL', 'gpt-4o-mini')}")
            print("   - Cambiar a: gpt-3.5-turbo o gpt-4")

        else:
            print("\n🔴 DIAGNÓSTICO: Error Desconocido")
            print("   - Revisar documentación de OpenAI")
...
def test_django_ai_services():
    ...
    try:
        ...
        if OPENAI_AVAILABLE:
            print("✅ AI SOP Generator: OpenAI disponible")
            ...
            if test_result and test_result.get('steps'):
                ...
                print("⚠️  Generación sin resultados")
                results.append(("AI SOP Generator", False))
        else:
            print("❌ OpenAI no disponible en módulo")
            results.append(("AI SOP Generator", False))
    ...
    try:
        ...
        if OPENAI_AVAILABLE:
            print("✅ AI Focus Helper: OpenAI disponible")
    ...
    try:
        ...
        if OPENAI_AVAILABLE:
            print("✅ Planner AI: OpenAI disponible")
    ...
    try:
        ...
        print("✅ Daily Plan AI: Módulo importable")
    ...
    try:
        ...
        print("✅ NLP Service: Módulo importable")
...
def generate_report(env_ok, deps_ok, api_ok, api_response, services_results):
    ...
    if all_ok:
        ...
        print("✅ Todos los servicios AI están disponibles")
        print("✅ Railway puede usar las funcionalidades AI")
        ...
        if not env_ok:
            ...
            print("   1. Ir a Railway Dashboard")
            print("   2. Seleccionar tu servicio")
            print("   3. Ir a 'Variables'")
            print("   4. Agregar: OPENAI_API_KEY = tu_api_key")
            print("   5. Obtener key en: https://platform.openai.com/api-keys")

        if not deps_ok:
            print("\n❌ ACCIÓN REQUERIDA: Instalar Dependencias")
            print("   1. Agregar a requirements.txt: openai>=1.0.0")
            print("   2. Railway instalará automáticamente en próximo deploy")

        if not api_ok:
            print("\n❌ ACCIÓN REQUERIDA: Verificar API Key")
            print("   1. Verificar que la key es válida")
            print("   2. Verificar que tienes créditos/quota")
            print("   3. Verificar conectividad en Railway")
    ...
    print("   - Railway Logs: railway logs")
