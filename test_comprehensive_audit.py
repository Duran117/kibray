...
def audit_tasks_module():
    ...
    print_test("Tasks", "Task List & Create Form GET", response.status_code == 200, f"Status: {response.status_code}")
    ...
    print_test(
        "Tasks",
        "Task Create POST with Priority/Due Date",
    ...
    if task:
        ...
        print_test(
            ...
            (
                ...
                if b"Seguimiento de tiempo" in response.content
                else "Missing time tracking"
            ),
        )
        ...
        print_test("Tasks", "Touch-up Board View", response.status_code == 200, f"Status: {response.status_code}")

        # Test 7: Touch-up board with priority filter
        response = client.get(f"/projects/{project.id}/touchups/?priority=high")
        print_test("Tasks", "Touch-up Board Priority Filter", response.status_code == 200, "Priority filter working")
...
def audit_daily_plans_module():
    ...
    print_test("Daily Plans", "Planning Dashboard", response.status_code == 200, f"Status: {response.status_code}")

    # Test 2: Create daily plan
    response = client.post(
        f"/planning/project/{project.id}/create/",
        {
            "plan_date": date.today().isoformat(),
        },
    )
    print_test(
        "Daily Plans", "Create Daily Plan", response.status_code in [200, 302], f"Status: {response.status_code}"
    )
    ...
    if plan:
        ...
        data = response.json() if response.status_code == 200 else {}
        print_test(
            "Daily Plans", "Fetch Weather Button", response.status_code == 200, f"Weather: {data.get('weather_data')}"
        )
...
def audit_materials_inventory():
    ...
    mat_request = MaterialRequest.objects.create(project=project, requested_by=user, status="draft", needed_when="now")
    ...
    print_test("Materials", "Material Request Created", True, f"Request #{mat_request.id}")
    ...
    response = client.post(f"/materials/requests/{mat_request.id}/create-expense/", {"total_amount": "500.00"})
    mat_request.refresh_from_db()
    print_test(
        "Materials",
    ...
    print_test("Inventory", "Low Stock Dashboard", response.status_code == 200, f"Status: {response.status_code}")
...
def audit_photos_module():
    ...
    print_test("Photos", "Photo List View", response.status_code == 200, f"Status: {response.status_code}")
    ...
    create_resp = client.post(
        f"/projects/{project.id}/photos/new/",
        {
            "room": "Living Room",
            "wall_ref": "North Wall",
            "photo_type": "progress",
    ...
    print_test(
        "Photos", "Create Photo POST", create_resp.status_code in [200, 302], f"Status: {create_resp.status_code}"
    )
    ...
    print_test("Photos", "GPS Location Fields", gps_ok, f"Lat: {photo.location_lat}, Lng: {photo.location_lng}")

    # Test 4: Verify visibility (Q18.4)
    visibility_ok = photo.visibility == "public"
    print_test("Photos", "Privacy Control", visibility_ok, f"Visibility: {photo.visibility}")

    # Test 5: Verify versioning fields (Q18.6)
    version_ok = photo.version == 1 and photo.is_current_version == True
    print_test(
        "Photos", "Versioning Fields", version_ok, f"Version: {photo.version}, Current: {photo.is_current_version}"
    )

    # Test 6: Verify caption (Q18.10)
    caption_ok = photo.caption == "Progress photo for living room"
    print_test("Photos", "Caption Field", caption_ok, f"Caption: {photo.caption[:30] if photo.caption else None}")
    ...
    print_test("Photos", "Visibility Filter", filter_ok, f"Status: {filter_resp.status_code}")
...
def audit_damage_reports():
    ...
    print_test("Damage Reports", "Damage Report List", response.status_code == 200, f"Status: {response.status_code}")
    ...
    print_test("Damage Reports", "Detail View", detail_resp.status_code == 200, f"Status: {detail_resp.status_code}")

    # Test 5: Change severity (Q21.7)
    report.change_severity("critical", user)
    report.refresh_from_db()
    severity_ok = (
        report.severity == "critical" and report.severity_changed_by == user and report.severity_changed_at is not None
    )
    print_test(
        "Damage Reports",
    ...
    status_resp = client.post(f"/damages/{report.id}/update-status/", {"status": "in_progress"})
    report.refresh_from_db()
    in_progress_ok = report.status == "in_progress" and report.in_progress_at is not None
    print_test("Damage Reports", "Status to In Progress", in_progress_ok, f"Started: {report.in_progress_at}")

    # Test 7: Resolve and check resolution time (Q21.9)
    resolve_resp = client.post(f"/damages/{report.id}/update-status/", {"status": "resolved"})
    report.refresh_from_db()
    ...
    filter_ok = filter_resp.status_code == 200
    print_test("Damage Reports", "Severity & Status Filters", filter_ok, f"Status: {filter_resp.status_code}")
...
def audit_inventory_movements_module():
    ...
    primary_loc, _ = InventoryLocation.objects.get_or_create(
    ...
    item = InventoryItem.objects.create(
        name="Audit Item", category="OTRO", unit="pcs", low_stock_threshold=Decimal("5")
    )
    ...
    receive_ok = (
        (recv_resp.status_code in [200, 302])
        and stock_after_receive
        and stock_after_receive.quantity == Decimal("10.00")
    )
    print_test(
        "Inventory Movements",
        "RECEIVE Movement",
        receive_ok,
        f"Qty: {stock_after_receive.quantity if stock_after_receive else 'None'}",
    )
    ...
    print_test(
        "Inventory Movements", "ISSUE Movement", issue_ok, f"Qty: {stock_issue.quantity if stock_issue else 'None'}"
    )
    ...
    neg_created = InventoryMovement.objects.filter(
        item=item, quantity=Decimal("100.00"), movement_type="ISSUE"
    ).exists()
    ...
    print_test(
        ...
        neg_ok,
        f"Status: {neg_resp.status_code}, Qty: {stock_neg.quantity if stock_neg else 'None'}",
    )
    ...
    transfer_ok = (
        (transfer_resp.status_code in [200, 302])
        and stock_primary
        and stock_secondary
        and stock_primary.quantity == Decimal("5.00")
        and stock_secondary.quantity == Decimal("2.00")
    )
    print_test(
        "Inventory Movements",
        "TRANSFER Movement",
        transfer_ok,
        f"Primary: {stock_primary.quantity if stock_primary else 'None'}, Secondary: {stock_secondary.quantity if stock_secondary else 'None'}",
    )
    ...
    print_test(
        "Inventory Movements", "ADJUST Movement", adjust_ok, f"Qty: {stock_adjust.quantity if stock_adjust else 'None'}"
    )
    ...
    history_ok = (
        history_resp.status_code == 200 and movements_count >= 4
    )  # RECEIVE + ISSUE + TRANSFER + ADJUST (negative attempt not applied)
...
def audit_payroll_module():
    ...
    for offset in range(5):
        ...
        TimeEntry.objects.get_or_create(
            employee=employee, project=project, date=d, start_time=dt.time(8, 0), end_time=dt.time(16, 30), defaults={}
        )
    ...
    update_ok = (
        update_resp.status_code in [200, 302]
        and record.reviewed
        and record.total_hours == Decimal("42.0")
        and record.overtime_hours == Decimal("2.0")
    )
    print_test(
        "Payroll",
        "Update Records POST",
    ...
    pay_resp = client.post(
        f"/payroll/record/{record.id}/pay/",
        {
    ...
    print_test("Payroll", "Record Payment Form", payment_ok, f"Paid: {amount_paid}, Due: {record.balance_due()}")

    # Test 7: Payment history view
    history_resp = client.get("/payroll/history/")
    history_ok = history_resp.status_code == 200
    print_test("Payroll", "Payment History View", history_ok, f"Status: {history_resp.status_code}")
...
def audit_color_samples_module():
    ...
    create_resp = client.post(
        f"/projects/{project.id}/colors/new/",
        {
            "code": "SW-7008",
            "name": "Alabaster",
    ...
    ok = (
        ...
        and sample.approved_by == user
        and sample.approved_at is not None
        and sample.sample_number
        and sample.approval_signature
    )
    print_test(
        "Color Samples",
        "Quick Approve Button",
        ok,
        f"Num: {sample.sample_number}, Sig: {sample.approval_signature[:12] if sample.approval_signature else None}",
    )

    # Test 4: Create second sample and reject
    reject_resp_create = client.post(
        f"/projects/{project.id}/colors/new/",
        {
            "code": "SW-0001",
            "name": "RejectMe",
    ...
    if second:
        reject_resp = client.post(
            f"/colors/sample/{second.id}/quick-action/", {"action": "reject", "reason": "Does not match palette"}
        )
        second.refresh_from_db()
        print_test(
            "Color Samples",
            "Quick Reject Button",
            reject_resp.status_code == 200 and second.status == "rejected" and second.rejection_reason,
            f"Reason: {second.rejection_reason}",
        )
    else:
...
def audit_floor_plans_module():
    ...
    create_resp = client.post(
        f"/projects/{project.id}/plans/new/",
        {
            "name": "Nivel Audit",
            "level": 0,
    ...
    print_test(
        "Floor Plans",
        "Add Pin & Auto Task",
        pin_resp.status_code in [200, 302] and auto_task_ok,
        f"PinResp: {pin_resp.status_code}, AutoTask: {auto_task_ok}",
    )
    ...
    if plan and hasattr(plan, "create_new_version"):
        ...
        print_test(
            ...
            migratable >= 0 and version_ok,
            f"Migratable Pins: {migratable}, New Version ID: {new_version.id}",
        )
