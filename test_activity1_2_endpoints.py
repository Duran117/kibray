...
from core.models import DailyPlan, Employee, MaterialRequest, Project, Task
...
def test_task_time_tracking():
    ...
    if response.status_code == 200:
        ...
        print_test("POST /tasks/<id>/start-tracking/", test_passed, f"Response: {data}")
    else:
        print_test("POST /tasks/<id>/start-tracking/", False, f"Status: {response.status_code}")
    ...
    if task.started_at:
        ...
        if response.status_code == 200:
            ...
            test_passed = data.get("success") == True
            print_test(
                "POST /tasks/<id>/stop-tracking/",
                test_passed,
                f"Elapsed: {data.get('elapsed_seconds')}s, Total: {data.get('total_hours')}h",
            )
        else:
            print_test("POST /tasks/<id>/stop-tracking/", False, f"Status: {response.status_code}")
...
def test_daily_plan_endpoints():
    ...
    if response.status_code == 200:
        data = response.json()
        print_test(
            "POST /planning/<id>/fetch-weather/", data.get("success") == True, f"Weather: {data.get('weather_data')}"
        )
    else:
        print_test("POST /planning/<id>/fetch-weather/", False, f"Status: {response.status_code}")
    ...
    if response.status_code == 200:
        data = response.json()
        print_test(
            "POST /planning/<id>/convert-activities/",
            "tasks_created" in data,
            f"Tasks created: {data.get('tasks_created', 0)}",
        )
    else:
        print_test("POST /planning/<id>/convert-activities/", False, f"Status: {response.status_code}")
    ...
    if response.status_code == 200:
        data = response.json()
        print_test(
            "GET /planning/<id>/productivity/",
            "productivity_score" in data,
            f"Score: {data.get('productivity_score')}%",
        )
    else:
        print_test("GET /planning/<id>/productivity/", False, f"Status: {response.status_code}")
...
def test_material_request_workflow():
    ...
    print_test(
        "POST /materials/requests/<id>/submit/",
    ...
    mat_request.refresh_from_db()
    print_test(
        "POST /materials/requests/<id>/create-expense/",
...
def test_inventory_endpoints():
    ...
    print_test("GET /inventory/low-stock/", response.status_code == 200, f"Status: {response.status_code}")
...
def test_notifications():
    ...
    print_test("Notifications created", recent_count > 0, f"Found {recent_count} recent notifications")
