...
def admin_user_create(request):
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            ...
            # Crear perfil
            Profile.objects.create(user=user, phone=request.POST.get("phone", ""), role=role)
...
def admin_group_detail(request, group_id):
    ...
    for perm in all_permissions:
        ...
        if model_name not in permissions_by_model:
            permissions_by_model[model_name] = {"label": perm.content_type.name, "permissions": []}
...
def admin_dashboard_main(request):
    ...
    context = {
        ...
        ).count(),  # Proyectos sin fecha de fin = activos
    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            try:
                ...
                try:
                    project.total_expenses = (total if isinstance(total, Decimal) else Decimal(str(total))).quantize(
                        Decimal("0.01")
                    )
                except Exception:
    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            try:
                ...
                try:
                    project.total_income = (total if isinstance(total, Decimal) else Decimal(str(total))).quantize(
                        Decimal("0.01")
                    )
                except Exception:
